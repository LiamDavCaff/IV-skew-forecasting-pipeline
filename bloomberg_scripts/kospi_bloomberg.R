...
spx_spot <- bdh("KOSPI2 Index",  "PX_LAST", as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
  rename(date = date, price = PX_LAST)

vix_df   <- bdh("VKOSPI Index", "PX_LAST", as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
  rename(date = date, vix = PX_LAST)

rfr_df   <- bdh("KRBO1M Index", "PX_LAST", as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
  rename(date = date, rfr_1m = PX_LAST)

tbl_df   <- bdh("GTKRW3M Govt", "PX_LAST", as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
  rename(date = date, tbl_3m = PX_LAST)

lty_df   <- bdh("GTKRW10Y Govt","PX_LAST", as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
  rename(date = date, lty = PX_LAST)

book_df  <- bdh("KOSPI2 Index", "BOOK_VAL_PER_SH",
                as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
  rename(date = date, book = BOOK_VAL_PER_SH)

infl_df  <- bdh("KOCPIYOY Index", "PX_LAST",
                as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
