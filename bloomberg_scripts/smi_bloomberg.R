###############################################################################
#   Swiss Market Index (SMI): 30-day implied-vol curve + macro / market panel
###############################################################################
library(dplyr)
library(tidyr)
library(purrr)
...
OUT_FILE <- file.path(DATA_DIR, "bbg_smi_data.rds")
...
# ── 3.  Market & macro data  ──────────────────────────────────────────
spx_tot  <- bdh("SMIC Index", "PX_LAST", as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
  rename(date = date, total_price = PX_LAST)

spx_spot <- bdh("SMI Index",  "PX_LAST", as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
  rename(date = date, price = PX_LAST)

vix_df   <- bdh("VSMI Index", "PX_LAST", as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
  rename(date = date, vix = PX_LAST)

rfr_df   <- bdh("SSARON Index", "PX_LAST", as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
  rename(date = date, rfr_1m = PX_LAST)

tbl_df   <- bdh("CHFI3M BGN Curncy", "PX_LAST", as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
  rename(date = date, tbl_3m = PX_LAST)

lty_df   <- bdh("GSWISS10 Index","PX_LAST", as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
  rename(date = date, lty = PX_LAST)

book_df  <- bdh("SMI Index", "BOOK_VAL_PER_SH",
                as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
  rename(date = date, book = BOOK_VAL_PER_SH)

infl_df  <- bdh("SZCPIYOY Index", "PX_LAST",
                as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
  rename(date = date, inflation_yoy = PX_LAST)

# ── 5.  SMI fundamentals (PE, EPS, PB) ─────────────────────────────────────
fund_df <- bdh("SMI Index",
