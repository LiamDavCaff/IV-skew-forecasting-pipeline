...
OUT_FILE <- file.path(DATA_DIR, "bbg_spx_data.rds")
...
# ── 2.  Pull IV surface (underlier = S&P 500) ───────────────────────────────
...
spx_spot <- bdh("SPX Index",  "PX_LAST", as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
  rename(date = date, price = PX_LAST)

vix_df   <- bdh("VIX Index", "PX_LAST", as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
...
book_df  <- bdh("SPX Index", "BOOK_VAL_PER_SH",
