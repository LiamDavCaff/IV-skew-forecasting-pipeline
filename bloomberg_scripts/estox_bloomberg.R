###############################################################################
#   EURO STOXX 50: 30-day implied-vol curve + macro / market panel
###############################################################################
library(dplyr)
library(tidyr)
library(purrr)
...
# ── 2.  Pull IV surface (underlier = EURO STOXX 50) ───────────────────────────────
get_iv_for_moneyness <- function(level_code){
  bdh(securities = "SX5E Index",              
...
# ── 3.  Market & macro data  ──────────────────────────────────────────
spx_tot  <- bdh("SX5GT Index", "PX_LAST", as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
  rename(date = date, total_price = PX_LAST)

spx_spot <- bdh("SX5E Index",  "PX_LAST", as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
  rename(date = date, price = PX_LAST)

vix_df   <- bdh("V2X Index", "PX_LAST", as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
  rename(date = date, vix = PX_LAST)

rfr_df   <- bdh("EUR001M Index", "PX_LAST", as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
  rename(date = date, rfr_1m = PX_LAST)

tbl_df   <- bdh("EUR003M Index", "PX_LAST", as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
  rename(date = date, tbl_3m = PX_LAST)

lty_df   <- bdh("GDBR10 Index","PX_LAST", as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
  rename(date = date, lty = PX_LAST)

book_df  <- bdh("SX5E Index", "BOOK_VAL_PER_SH",
                as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
  rename(date = date, book = BOOK_VAL_PER_SH)

infl_df  <- bdh("ECCPEMUY Index", "PX_LAST",
                as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
  rename(date = date, inflation_yoy = PX_LAST)

# ── 5.  ESTOX fundamentals (PE, EPS, PB) ─────────────────────────────────────
fund_df <- bdh("SX5E Index",
