###############################################################################
#   Nifty-50: 30-day implied-vol curve + macro / market panel
###############################################################################
library(dplyr)
library(tidyr)
library(purrr)
...
OUT_FILE <- file.path(DATA_DIR, "bbg_nifty_data.rds")
...
# ── 2.  Pull IV surface (underlier = NIFTY) ───────────────────────────────
get_iv_for_moneyness <- function(level_code){
  bdh(securities = "NIFTY Index",              
...
# ── 3.  Market & macro data ──────────────────────────────────────────
spx_tot  <- bdh("NIFTYTR Index", "PX_LAST", as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
  rename(date = date, total_price = PX_LAST)

spx_spot <- bdh("NIFTY Index",  "PX_LAST", as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
  rename(date = date, price = PX_LAST)

vix_df   <- bdh("INVIXN Index", "PX_LAST", as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
  rename(date = date, vix = PX_LAST)

rfr_df   <- bdh("IRSWOA BGN Curncy", "PX_LAST", as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
  rename(date = date, rfr_1m = PX_LAST)

tbl_df   <- bdh("GIND3M Index", "PX_LAST", as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
  rename(date = date, tbl_3m = PX_LAST)

lty_df   <- bdh("GIND10YR Index","PX_LAST", as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
  rename(date = date, lty = PX_LAST)

book_df  <- bdh("NIFTY Index", "BOOK_VAL_PER_SH",
                as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
  rename(date = date, book = BOOK_VAL_PER_SH)

infl_df  <- bdh("INFUTOTY  Index", "PX_LAST",
                as.Date("2005-01-01"), as.Date("2025-07-31")) %>% 
  rename(date = date, inflation_yoy = PX_LAST)

# ── 5.  NIFTY fundamentals (PE, EPS, PB) ─────────────────────────────────────
fund_df <- bdh("NIFTY Index",
...
# ── 6.  Merge everything ─────────────────────────────────────────────────────
master_df <- iv_wide %>% 
  left_join(spx_tot,  by = "date") %>% 
  left_join(spx_spot, by = "date") %>% 
